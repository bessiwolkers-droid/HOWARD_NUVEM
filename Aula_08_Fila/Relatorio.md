...
Se a identificação não existir, a resposta é `404` em vez de devolver um corpo vazio com código `200`. Isso deixa clara a diferença entre uma tarefa que ainda não terminou e uma identificação que nunca foi criada.
...
A documentação automática do FastAPI fica em `http://localhost:8000/docs`. Em outro terminal, dentro da mesma pasta, inicie o worker:

```bash
python3 worker.py
```

Para testar o caminho assíncrono pelo terminal, usei o cliente:
